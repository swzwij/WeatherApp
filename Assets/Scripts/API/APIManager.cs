...
public class APIManager : SingletonBehaviour<APIManager>
{
    ...
    public void GetHourlyRain(double latitude, double longitude, int days, string startDate, string endDate, Action<HourlyRainData> onComplete = null, Action onFailure = null)
    {
        string url = $"latitude={latitude}&longitude={longitude}&hourly=precipitation,weathercode&forecast_days={1}&start_date={startDate}&end_date={endDate}";
        StartCoroutine(HourlyRainCall(_baseURL + url, onComplete, onFailure));
    }

    private IEnumerator HourlyRainCall(string url, Action<HourlyRainData> onComplete, Action onFailure)
    {
        ...
        HourlyRainData response = JsonUtility.FromJson<HourlyRainData>(request.downloadHandler.text);
